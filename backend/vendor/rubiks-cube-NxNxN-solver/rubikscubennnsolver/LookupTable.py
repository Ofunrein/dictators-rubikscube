...
def download_file_if_needed(filename: str) -> None:
    ...
    if not os.path.exists(filename):
        ...
        if not os.path.exists(filename_gz):
            ...
            logger.info(f"Downloading table via Python urllib: {url}")
            try:
                with urllib.request.urlopen(url) as response, open(filename_gz, "wb") as target_fh:
                ...
                logger.warning(
                    "SSL certificate verification failed downloading %s; retrying with an unverified context.",
                    url,
                )
                insecure_context = ssl._create_unverified_context()
                with urllib.request.urlopen(url, context=insecure_context) as response, open(filename_gz, "wb") as target_fh:
                    shutil.copyfileobj(response, target_fh)

            if not os.path.exists(filename_gz):
                raise Exception(f"failed to download {filename_gz} via {url}")

        logger.info(f"gunzip {filename_gz} via Python gzip")
        with gzip.open(filename_gz, "rb") as source_fh, open(filename, "wb") as target_fh:
