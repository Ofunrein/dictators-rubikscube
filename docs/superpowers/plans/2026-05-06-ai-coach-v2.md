...
**Goal:** Replace the mock AI coach with a real Groq-powered coach (LLaMA 3.3-70B) that gives terse, cube-state-aware coaching; fix solve mode to use the real WASM/Python solver.

**Architecture:** Port `aiCoach.ts` to `aiCoach.js` (plain JS, no build step), wire it into `routes.js` replacing `aiCoachRuntime.js`, and add solver dispatch for all three cube sizes. Groq is called via the OpenAI-compatible chat completions endpoint using native `fetch`. Falls back to a smart deterministic mock on any failure.

**Tech Stack:** Node.js 18+ (native fetch), Groq API (OpenAI-compatible), existing WASM solver (3x3), existing Python solver (2x2/4x4).

**IMPORTANT:** Push to `github` remote (`Ofunrein/dictators-rubikscube`) only. NEVER push to `origin` (Bitbucket).
...
| File | Action |
|------|--------|
| `backend/api/src/lib/aiCoach.js` | **Create** — full port of `aiCoach.ts` to plain JS |
| `backend/api/src/routes.js` | **Modify** — swap import from `aiCoachRuntime.js` → `aiCoach.js`; pass solver to coach |
| `backend/api/.env.example` | **Modify** — document Groq env vars |
| `backend/api/src/lib/aiCoachRuntime.js` | **Leave untouched** — kept as reference, no longer imported |

---

## Task 1: Create `aiCoach.js` — tutor signals + mock coach

**Files:**
- Create: `backend/api/src/lib/aiCoach.js`

- [ ] **Step 1.1: Create the file with move helpers and tutor signal functions**
...
- [ ] **Step 1.2: Add stage detection and question-aware replies**
...
- [ ] **Step 1.3: Add string utilities and response quality enforcement**
...
- [ ] **Step 1.4: Add `createMockCoachMessage` (smart deterministic fallback)**
...
```bash
cd ~/Downloads/the-dictators
git add backend/api/src/lib/aiCoach.js
git commit -m "feat(coach): add aiCoach.js - tutor signals, mock coach, quality enforcement"
git push github main
```

---

## Task 2: Add Groq API call to `aiCoach.js`

**Files:**
- Modify: `backend/api/src/lib/aiCoach.js`

- [ ] **Step 2.1: Add prompt builder**

Append to `backend/api/src/lib/aiCoach.js`:
...
function buildCoachPrompt(payload) {
  const modeBehavior = {
    ...
    solve: ['Provide an actionable sequence of legal cube move tokens.', 'Always populate moves array if any sequence is known.', 'Keep explanation short and execution-focused.'],
    explain: ['Explain the underlying principle behind prior advice.', 'Include cause/effect and one common mistake to avoid.', 'Make it beginner-friendly and concrete.'],
  };
...
- [ ] **Step 2.2: Add Groq API caller and response normalizer**
...
- [ ] **Step 2.3: Add `generateAiCoachResult` — main exported entry point**
...
export async function generateAiCoachResult(payload, solveWithWasm, solveWithPython) {
  ...
  // Solve mode: use real solver, not reverse-history
  if (payload.mode === 'solve') {
    const size = payload.context.cubeState
      ? Object.values(payload.context.cubeState)[0]?.length === 9 ? 3
        : Object.values(payload.context.cubeState)[0]?.length === 4 ? 2 : 3
      : 3;
    try {
      let result;
      if (size === 3) {
        result = await solveWithWasm(payload.context.cubeState);
        const moves = Array.isArray(result?.moves) ? result.moves : (Array.isArray(result) ? result : []);
        return {
          coachMessage: {
            id: 'coach_solve_wasm',
  ...
  // hint / guide / explain: use Groq if available
...
- [ ] **Step 2.4: Test the Groq call locally**

Start the backend:
```bash
cd ~/Downloads/the-dictators
AI_PROVIDER=openai AI_BASE_URL=https://api.groq.com/openai AI_MODEL=llama-3.3-70b-versatile AI_PROVIDER_API_KEY=<GROQ_API_KEY_FROM_CLAUDE_MD> node backend/api/src/server.js &
sleep 2
```

Send a hint request:
```bash
curl -s -X POST http://localhost:5200/v1/ai/help \
  -H "Content-Type: application/json" \
  -d '{"mode":"hint","message":"what should I do?","context":{"cubeState":{"U":["W","R","W","W","W","B","W","W","W"],"D":["Y","Y","Y","Y","Y","Y","Y","Y","Y"],"F":["G","G","G","G","G","G","G","G","G"],"B":["B","B","B","B","B","B","B","B","B"],"L":["O","O","O","O","O","O","O","O","O"],"R":["R","R","R","R","R","R","R","R","R"]},"moveHistory":["R","U","R"],"scramble":["R","U","R"],"timerMs":5000,"idleMs":0,"solveDepth":3,"queueActive":false,"isSolved":false}}' | python3 -m json.tool
```

Expected: response with `meta.isMock: false` and a real coaching hint in `coachMessage.content`.

Kill test server: `pkill -f "node backend/api/src/server.js"`
...
cd ~/Downloads/the-dictators
git add backend/api/src/lib/aiCoach.js
git commit -m "feat(coach): add Groq API call, prompt builder, response normalizer"
git push github main
```

---

## Task 3: Wire `aiCoach.js` into `routes.js`

**Files:**
- Modify: `backend/api/src/routes.js`

- [ ] **Step 3.1: Swap the import**

In `backend/api/src/routes.js`, replace lines 31-36:
...
- [ ] **Step 3.2: Pass solvers to `generateAiCoachResult` in `handleAiHelpRoute`**

In `backend/api/src/routes.js`, update `handleAiHelpRoute` (around line 245):
...
- [ ] **Step 3.3: Verify `solveCubeMoveListWithWasm` is imported in routes.js**

Check line 40-42 of `routes.js` — it should already import `solveCubeMoveListWithWasm`:
```js
import {
  solveCubeMoveListWithWasm,
...
If missing, add `solveCubeMoveListWithWasm` to that import.

- [ ] **Step 3.4: Smoke test the full route**
...
curl -s -X POST http://localhost:5200/v1/ai/help \
  -H "Content-Type: application/json" \
  -d '{"mode":"hint","context":{"cubeState":{"U":["W","R","W","W","W","B","W","W","W"],"D":["Y","Y","Y","Y","Y","Y","Y","Y","Y"],"F":["G","G","G","G","G","G","G","G","G"],"B":["B","B","B","B","B","B","B","B","B"],"L":["O","O","O","O","O","O","O","O","O"],"R":["R","R","R","R","R","R","R","R","R"]},"moveHistory":[],"scramble":[],"timerMs":0,"idleMs":0,"solveDepth":0,"queueActive":false,"isSolved":false}}' | python3 -m json.tool

pkill -f "node backend/api/src/server.js"
```

Expected: `meta.isMock: false`, real AI response in `coachMessage.content`.

- [ ] **Step 3.5: Commit**

```bash
cd ~/Downloads/the-dictators
git add backend/api/src/routes.js
git commit -m "feat(coach): wire aiCoach.js into routes.js, pass real solvers to generateAiCoachResult"
git push github main
```

---

## Task 4: Update `.env.example` and set Vercel env vars
...
Replace `backend/api/.env.example` content:

```
NODE_ENV=development
PORT=4011
DATABASE_URL=postgresql://postgres:postgres@localhost:5432/rubiks_app?schema=public
FRONTEND_ORIGIN=http://localhost:5173

# AI Coach — set AI_PROVIDER=openai + AI_PROVIDER_API_KEY to enable real coaching.
# Free tier: Groq (https://console.groq.com). Leave AI_PROVIDER=mock for offline/demo.
AI_PROVIDER=mock
...
In Vercel dashboard for the dictators project, add:
```
AI_PROVIDER=openai
AI_BASE_URL=https://api.groq.com/openai
AI_MODEL=llama-3.3-70b-versatile
AI_PROVIDER_API_KEY=<GROQ_API_KEY_FROM_CLAUDE_MD>
```
...
```bash
cd ~/Downloads/the-dictators
git add backend/api/.env.example
git commit -m "docs(coach): update .env.example with Groq AI coach env vars"
git push github main
```
...
**Spec coverage check:**
- ✅ Port `aiCoach.ts` → `aiCoach.js` — Task 1 + 2
- ✅ Groq API (hint/guide/explain) — Task 2
- ✅ Solve mode → real solver (WASM 3x3, Python 2x2/4x4) — Task 2, Step 2.3
- ✅ Smart deterministic fallback on any failure — Task 2, Step 2.3 (`makeMockResult`)
- ✅ Terse responses enforced — Task 1, Step 1.3 (`enforceModeQuality`)
- ✅ Tutor signals (loop, idle, dominant face) — Task 1, Steps 1.1-1.2
- ✅ Wire into routes.js — Task 3
- ✅ Env vars documented — Task 4
- ✅ No TypeScript, no new npm packages — native `fetch` throughout
- ✅ Push to `github` only — every commit uses `git push github main`

**No placeholders found.** All code blocks are complete.

**Type consistency:** `generateAiCoachResult(payload, solveWithWasm, solveWithPython)` — defined in Task 2 Step 2.3, called in Task 3 Step 3.2. Match confirmed.
