...
export default function TutorialPanel({
    ...
    <>
      {isThreeByThree ? (
            ...
            >
              <span
                className={`mt-0.5 shrink-0 font-mono text-sm font-bold ${
                  tutorialStep === index ? 'text-dictator-red' : 'text-white/90'
                }`}
              >
                {String(index + 1).padStart(2, '0')}
              </span>
              <div>
                <p className="mb-1 font-heading text-base font-bold text-white">{step.title}</p>
                {tutorialStep === index && (
        ...
        <div className="flex flex-col gap-4 p-5 sm:p-6">
          <div className="rounded-xl border border-dictator-chrome/10 bg-[#111] p-4">
            <p className="mb-2 font-mono text-[11px] uppercase tracking-widest text-white">Mode Summary</p>
          ...
          <div className="rounded-xl border border-dictator-chrome/10 bg-[#111] p-4">
            <p className="mb-2 font-mono text-[11px] uppercase tracking-widest text-white">Why The Guide Changes</p>
            <p className="font-body text-sm leading-relaxed text-white/95">
          ...
          <div className="rounded-xl border border-dictator-chrome/10 bg-[#111] p-4">
            <p className="mb-2 font-mono text-[11px] uppercase tracking-widest text-white">Current Limits</p>
            <p className="font-body text-sm leading-relaxed text-white/95">
      ...
      <div className="border-t border-dictator-chrome/10 p-5 sm:p-6">
        <p className="mb-4 font-mono text-[11px] uppercase tracking-widest text-white">
          Controls
        </p>
        <div className="flex flex-col gap-3">
          {SIMULATOR_CONTROL_SECTIONS.map((section, index) => {
            ...
            return (
              <div
                key={section.title}
                className={`rounded-xl border bg-[#111] transition-colors ${
                  expanded
                    ? 'border-dictator-red/40'
                    : 'border-dictator-chrome/10'
                }`}
              >
                <button
                  type="button"
                ...
                >
                  <div>
                    <p className="font-heading text-base font-bold text-white">{section.title}</p>
                  </div>
                  <ChevronRight
                    size={16}
                    className={`shrink-0 text-white/90 transition-transform ${expanded ? 'rotate-90 text-dictator-red' : ''}`}
                  />
                </button>
                {expanded && (
      ...
      {isThreeByThree && (
        <div className="mt-auto border-t border-dictator-chrome/10 p-5 sm:p-6">
          <p className="mb-4 font-mono text-[11px] uppercase tracking-widest text-white">
            Quick Algorithms
          </p>
          <div className="flex flex-col gap-3">
            {QUICK_ALGORITHMS.map(({ name, moves }) => (
              <div key={name} className="rounded-xl border border-dictator-chrome/10 bg-[#111] p-3">
                <p className="mb-1 font-mono text-[11px] uppercase tracking-widest text-white">{name}</p>
                ...
                <button
                  ...
                  className={`mt-2 flex items-center gap-1 font-mono text-[11px] transition-colors
                    ${queueActive ? 'cursor-not-allowed text-white/90' : 'text-white hover:text-dictator-red'}`}
                >
  ...
  if (isDrawer) {
    return (
      <>
        ...
        />
        <aside
          className={`fixed inset-y-0 right-0 z-50 flex w-[min(92vw,360px)] max-w-full flex-col overflow-y-auto border-l border-dictator-chrome/10 bg-[#0A0A0A] shadow-2xl transition-transform duration-300 lg:hidden ${
            drawerOpen ? 'translate-x-0' : 'translate-x-full'
          }`}
          aria-hidden={!drawerOpen}
        >
          <div className="flex items-center justify-between border-b border-dictator-chrome/10 px-5 py-4">
            <div>
              <p className="font-mono text-[11px] uppercase tracking-widest text-white/90">Guide</p>
            ...
            <button
              type="button"
              onClick={onClose}
              className="rounded-full border border-dictator-chrome/15 p-2 text-white/70 transition-colors hover:border-dictator-red/40 hover:text-dictator-red"
  ...
  return (
    <aside className="hidden lg:flex lg:w-[260px] lg:min-w-[260px] xl:w-[320px] xl:min-w-[320px] flex-col overflow-y-auto border-l border-dictator-chrome/10 bg-[#0A0A0A]">
      <div className="border-b border-dictator-chrome/10 p-5 sm:p-6">
        <p className="mb-1 font-mono text-[11px] uppercase tracking-widest text-white">// LEARN</p>
